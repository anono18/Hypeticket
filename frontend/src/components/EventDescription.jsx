...
const EventDescription = (props) => {
    ...
    const handleReserve = async () => {
        ...
        setEmailDialog(true); // Show the email input dialog
    };
    ...
    const handleEmailSubmit = async () => {
        ...
        try {
            await axios.post(
                'http://localhost:4000/reserve',
                { ticketId: selectedTicket, quantity, email: reservationEmail },
                { headers: { 'auth-token': token } }
            );
    ...
    return (
        <div className='max-padd-container mt-20 grid lg:flex lg:gap-8 sm:grid-cols-1 lg:grid-cols-2'>
            ...
            {showDialog && (
                <div className='fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50'>
                    <div className='bg-white p-6 rounded-lg shadow-lg w-full max-w-sm'>
                        {step === 1 && (
                            <>
                                <p className='text-sm text-gray-700 mb-4'>Entrez votre numéro de téléphone:</p>
                                <input
                                    type="text"
                                    value={phoneNumber}
                                    onChange={(e) => setPhoneNumber(e.target.value)}
                        ...
                        {step === 3 && (
                            <>
                                <p className='text-sm text-gray-700 mb-4'>Entrez le code de sécurité:</p>
                                <input
                                    type="password"
                                    value={securityCode}
                                    onChange={(e) => setSecurityCode(e.target.value)}
            ...
            {emailDialog && (
                <div className='fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50'>
                    <div className='bg-white p-6 rounded-lg shadow-lg w-full max-w-sm'>
                        <p className='text-sm text-gray-700 mb-4'>Veuillez entrer votre adresse e-mail pour recevoir le code de réservation:</p>
                        <input
                            type="email"
            ...
            {emailConfirmation && (
                <div className='fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50'>
                    <div className='bg-white p-6 rounded-lg shadow-lg w-full max-w-sm'>
                        <p className='text-lg font-semibold text-center'>Code de réservation envoyé avec succès !</p>
                        <p className='text-sm text-gray-700 text-center mt-4'>Le code a été envoyé à {reservationEmail}.</p>
                        <button
